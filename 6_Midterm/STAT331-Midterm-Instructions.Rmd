...
-  Submit both your `.Rmd` and `.html` files on Canvas by the end of the
  allotted time. 
- Only your knitted HTML file will be graded, so make sure the knitted output
contains all your answers!
  
### The Dataset

Parts Two, Three, and Four of this exam pertain to the dataset 
`Minimum Wage Data.csv` posted in the Midterm Module on Canvas.
...
```{r, include = FALSE}
x <- factor(rep(LETTERS[1:7], 
                times = c(40, 10, 10, 10, 10, 10, 10)))

x %>% 
  table()

x %>% fct_lump_prop(0.1) %>% table()
x %>% fct_lump_min(10) %>% table()
...
year? Your output should display the respective state and it's minimum wage.
(Use the `Effective.Minimum.Wage` dollar amount.)__

*You do not need to output __all__ of the states. Rather, include your
...
__3. Which states / territories had the largest difference in minimum wage
(difference between high and low values) in each year? Your output should
display the respective state and it's difference in wages. (Use the Department
of Labor values.)__
...
*The aesthetics of your plot should be mindfully chosen. For example, what 
method(s) will you use so the reader can easily compare the minimum wages for
each state? Where is the most aesthetically appealing place for the legend to 
go? What are good axis labels? What are nice colors that are easy to
differentiate?*
...
min_wage %>%
  ...
  labs(x = "Year", 
       ...
       title = "Effective Minimum Wage", 
       subtitle = "(Lesser of State Minimum & Federal Minimum") + 
...
*Keep in mind your report should look professional! Use the tools you've learned
to create a nice report that is easy to read! In your report, there should be
either no code output, or code that can be toggled to appear / disappear.* 

### How do the minimum wages differ by region?

Decide on your own regional grouping for the states. Your grouping is required
to have __*at least* three groups__. For example, you might consider East vs.
Central vs. West or something more nuanced. You are welcome to reference any
internet source to determine your grouping.
...
tables) that tells a clear story. Your write-up must include *at least* one 
visualization and *at least* one table of summary statistics.
...
For your oral exam, you are tasked with explaining the code you produced to 
create regional groupings. 

During your exam, you are expected to describe what each line of code is doing.
That means you need to describe: 

- What action a function performs
- What is being passed into the function as an input
- How function arguments are used
- What is being output by the function 

For example, here is my recording talking through the following three lines of
code <https://voca.ro/12APWFXv5dkx>. 
