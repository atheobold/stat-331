...
_Missing values were replaced with mean values in some of the variables in the 
...
young <- hiphop %>% 
  filter(age < 20) %>% 
  group_by(word) %>% 
  ## A more specific summarize() function, where we summarize for a specific variable
  summarize_at(vars(familiarity), list(mean)) 
  ## Although I am generally opposed to taking the mean of Likert data, this gives an "easy" answer


## Most familiar
young %>% 
  top_n(n = 1, familiarity) %>% 
  pull(word)

## Least familiar
young %>% 
  top_n(n = 1, desc(familiarity)) %>% 
  pull(word)
```
...
genres %>% 
  ...
  summarize_at(vars(intl:hiphop), list(mean)) %>% 
  ## For all of the music genres, find the difference in the sexes
  summarise_at(vars(intl:hiphop), list(diff)) %>% 
  ## Make the differences all positive to compare magnitudes
  summarise_at(vars(intl:hiphop), list(abs)) %>% 
  which.max()
...
genres %>% 
  ...
  ## For all of the music genres, find the mean for each ethnic group
  summarize_at(vars(intl:hiphop), list(mean)) %>% 
  ## For all of the music genres, find the difference in the ethnic groups
  summarise_at(vars(intl:hiphop), list(diff)) %>% 
  ## Make the differences all positive to compare magnitudes
  summarise_at(vars(intl:hiphop), list(abs)) %>% 
  which.max()
...
```{r}
genres %>% 
  mutate(drinking_age = if_else(age < 21, "under 21", "21 or over")) %>% 
  group_by(drinking_age) %>%
  ## For all of the music genres, find the mean for each age group
  summarize_at(vars(intl:hiphop), list(mean)) %>% 
  ## For all of the music genres, find the difference in the age groups
  summarise_at(vars(intl:hiphop), list(diff)) %>%
  ## Make the differences all positive to compare magnitudes
  summarise_at(vars(intl:hiphop), list(abs)) %>% 
  which.max()
