...
The `{{phrase_col}}` part, which I provided for you, lets you use column names as arguments to a function. **Do not** delete that line of code!

**Getting the days**  
The `ordinal()` function from the **english** package could be helpful when coverting the `line` input (e.g., 1) to a word (e.g., "first").
...
There **should not** be iteration *inside* your function. We will use iteration *outside* the function, to sing each day of the song. 

**Singing all the gifts***
Remember, each day, you get the gift for that day *and* all the prior days. Also, remember that these gifts go in reverse order! (e.g., two turtle doves and a partridge in a pear tree). 
...
<!-- [lecture slides from Tuesday](../Synchronous/pa_review.html)  -->
:::
...
```{r}
## Reuse your code from the Practice Activity here, so you have the Full.Phrase column
xmas2 <- xmas %>%
  mutate(Full.Phrase = pmap_chr(.l = list(), 
                                .f = make_phrase)
         )

## Code to iterate your sing_line() function for the 12 days of Christmas
...
-   Not using the `map()`  to iterate your function.
