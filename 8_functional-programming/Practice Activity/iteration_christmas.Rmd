...
Once you have a copy of the repository, you can add, commit, and push the
changes you make to **your** repository. 
...
The `Day.in.Words` variable isn't quite what you want! You want `12`
to say `"twelve"` **not** `"twelfth"`. I've added a line of code below to make a
`day.num` variable, of the numbered days (using the **english** package). 

The code below uses your function, the `day.num` variable, and the associated
variables in the dataset to create a `Full.Phrase` column. Note the code I
provided uses `pmap()`, which assumes your `make_phrase()` function is not 
vectorized!
