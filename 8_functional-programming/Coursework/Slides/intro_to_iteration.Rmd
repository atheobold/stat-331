...
```{r}
penguins %>% 
  mutate(bill_length_mm = standardize(bill_length_mm), 
         bill_depth_mm = standardize(bill_depth_mm), 
         flipper_length_mm = standardize(flipper_length_mm), 
...
```{r}
penguins %>% 
  mutate_at(vars(bill_length_mm:body_mass_g), standardize)
```

---

.larger[Iteration Paradigms]
