...
Yes, it might take some time to get used to the idea of having a `for()`-loop built into a function, but it’s worth the investment. In the rest of this coursework, you’ll learn about and use the **purrr**^[I fully support more R packages being cat themed.] package, which houses functions that eliminate the need for many common `for()`-loops. 
