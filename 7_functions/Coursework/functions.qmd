...
copied and pasted your code __more than twice__ (i.e. you now have three copies
of the same code)" (Wickham & Grolemund, 2020). 
...
<!-- `r emo::ji("play button")` [Recommended Video: The gritty details of function arguments -- 12 minutes](link) -->

<!-- `r emo::ji("play button")` [Recommended Video: Do you need a `return()` statement? -- 9 minutes](link) -->
...
::: {.callout-tip}
I **highly recommend** you work through _every_ tutorial except Loops! 
:::
...
function should take a single argument (`x`) and multiply the input by 7. 
...
v) An error defined in a different function, which is called inside the 
...
Once you've made your function, try it out on the built-in dataset `mtcars`. In
these data the car information is included as rownames. Use the
`rownames_to_column()`function to create a new column in the data called `make`
that gives the make and model of each car. 
...
4. Functions should be **self-contained**.
    ...
    + *Functions should __never__ load or install packages!*
