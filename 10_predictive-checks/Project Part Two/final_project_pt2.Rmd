...
With your chosen model, generate predictions using the `predict()` function.
Then, add random errors to the predictions, using the residual standard error 
estimated from the linear regression model (acquired with `sigma()`). 
...
simulated data (that you made at the end of Part One). 

Plot the visualization of the observed data and the simulated data side-by-side. Discuss how the simulated data are similar and / or different to the observed
data. 
...
instructional expenditures and test scores. 

Regress the observed dataset against each simulated dataset 
(e.g. `lm(observed ~ simulated)`), and keep the $R^2$ from the regression. Keep 
...
model accuracy describe the relationships present in the data?). 
