...
> It is difficult to include in a [statistical model] all of one’s
...
distribution with mean 0 and standard deviation $\sigma$ ($\epsilon \sim N(0, \sigma)$). 
...
predicted from the linear regression. Moreover, when we randomly generating
these errors, we better understand what the "normality," "equal variance,"
and "independence" conditions for linear regression mean.
...
like values for the same country across multiple years, then it's pretty likely
that I can guess the value of a variable in 1991 from knowing the same
variable's value in 1990. Since we are using a linear regression to model the relationship between the explanatory variable(s)
...
When simulating data that come from a linear regression, why should the default
value for the `mean` be 0? (when passed into the `rnorm()` function)
...
`bill_length` and `bill_depth` are radically different from the 
...
underestimates, but the values are not terribly close to the line implying that
the simulated data are not very close to what was observed.
...
regression would do. I like $R^2,$ since it has an easy reference value 
for what I would expect if my model did a good job ($R^2 \approx 1$). 

I like the `glance()` function from the **broom** package, since it produces 
a nice table output of the summary measures from a linear regression. 
...
I can then use the column names of the table to `select()` the `r.squared`
column. 
...
In this plot, we see that the simulated datasets have $R^2$ vales between 0.03
and 0. To me, this indicates that the data simulated under this statistical 
model **are not at all** similar to what was observed. On average, our simulated
data account for *at most* 3% of the variability in the observed penguin bill
lengths. 
