...
<!-- *Note:* The `lm()` function **does not** have the data as the first argument,  -->
<!-- which is why there is a `.` in the `data =` argument. This tells `lm()` where -->
<!-- to insert the data that I piped in.  -->
...
This is what is output when you just call the name of the linear model object 
you created (`my_model`). Notice, the output doesn't give you much information
...
This is what is output when you use the `summary()` function on a linear model
object. Notice, the output gives you a lot of information, some of which is
really not that useful. And, the output is quite messy!
...
The `tidy()` function from the broom package takes a linear model object and 
puts the "important" information into a tidy tibble output. 
...
If you are sad that you no longer have the statistics about the model fit
(e.g., R-squared, adjusted R-squared, $\sigma$), you can use the `glance()`
function from the broom package to grab those! 
...
a) In the code above, what does the `data = .` argument in the `lm()` function do?
...
b) Penguin Mopsy has a bill that is 5mm deeper than Penguin Gidget. How much
shorter do you expect Penguin Mopsy's bill to be?
...
*(Hint: Think about what needs to change in the aesthetic of the plot featured
previously?)*

b) Which model of the two models explains the most variance in `bill_length_mm`?

c) Do the three species of penguin have the same bill shape (i.e., the
