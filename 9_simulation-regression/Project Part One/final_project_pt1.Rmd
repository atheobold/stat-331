...
schools <- read_xlsx(here::here("9_simulation-regression", 
                                "Project Part One", 
                                "US_educational_data_project.xlsx"))
```
...
schools_small <- schools_small %>% 
  mutate(year = word(PRIMARY_KEY, start = 1, end = 1, sep = "_"), 
         state = word(PRIMARY_KEY, start = 2, end = 2, sep = "_")) %>% 
  select(-PRIMARY_KEY, year, state, everything())
...
schools_small %>% 
  ...
  ggplot(aes(x = year, y = INSTRUCTION_EXPENDITURE)) + 
...
investigating the relationship between instructional expenditures and reading test scores. 
