...
Piping is an infix operator (e.g., `2 + 2`) that pipes the LHS of the pipe into the **first** argument of the function on RHS of the pipe
...
With `%>%`, if the LHS **should not** be used in the **first** argument, a `.` can be used to reference where the LHS should go.
:::
:::
...
::: {style="font-size: 0.75em; color: #000000;"}
`|>` **does not** have a placeholder. However, as of R 4.2, `|>` can use `_` as a placeholder but only with **named** arguments.
:::
:::
:::
...
::: {style="font-size: 0.85em; color: #FFFFFF;"}
-   `r` stands for for **random sampling**: generate a random value from a specific distribution
:::

. . .

::: {style="font-size: 0.85em; color: #FFFFFF;"}
-   `p` for **probability** (CDF): i.e. compute the probability of observing a value **less than** `x` (cumulative)
:::
...
::: {style="font-size: 0.85em; color: #FFFFFF;"}
-   `d` stands for **density** (PDF): compute the height of distribution curve (absolute)
:::
