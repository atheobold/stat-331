...
- Only ask the instructor when it's a team question
- Think and work together--don't divide up the work
- work within your group--no crosstalk with other groups
...
```{r, echo = FALSE}
flair("seq", eval = FALSE, pattern = "seq")
```
...
```{r, echo = FALSE}
flair("seq", eval = FALSE, pattern = c("from", "to"))
```
...
```{r, echo = FALSE}
flair("seq", eval = FALSE, pattern = "by = 2")
```
