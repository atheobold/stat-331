...
1. In order to create the plot we want, `month` needs to be a character
variable, not a numeric variable. Write the code to convert `month` into a 
character data type. 

```{r, eval = FALSE}
# Option 1 -- base R
nycflights <- as.character(nycflights$month)
```

```{r}
# Option 2 -- dplyr & forcats
nycflights <- nycflights %>% 
  mutate(month = as_factor(month))

```

2. Create a visualization of the relationship between the departure delay 
...
3. Add the origin of the flight (`origin`) to your visualization. 
